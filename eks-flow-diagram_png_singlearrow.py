...
def create_png_diagram(df, resource_mapping, pod_mapping):
    """Generate a static PNG diagram using Diagrams with better layout, unique connections, and bidirectional arrows."""
    ...
    with Diagram("EKS Architecture", show=False, outformat="png", filename="eks_architecture"):
        ...
        for _, row in df.iterrows():
            ...
            if src_ip in nodes and dst_ip in nodes:
                ...
                # If reverse connection exists, use a **double-headed arrow** and store only once
        ...
        # **Draw edges**
        for src_ip, dst_ip in unique_connections:
            nodes[src_ip] >> nodes[dst_ip]  # Normal single-direction arrow

        for src_ip, dst_ip in bidirectional_connections:
            nodes[src_ip] << nodes[dst_ip]  # **Bidirectional double-headed arrow**
