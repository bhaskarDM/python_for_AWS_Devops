...
def load_vpc_flow_logs(file_path, resource_mapping):
    ...
    df = pd.read_csv(file_path, names=columns, skiprows=1, na_values="-")
    df = df.drop_duplicates(subset=["pkt-srcaddr", "pkt-dstaddr"])
    df = df.dropna(subset=["pkt-srcaddr", "pkt-dstaddr"])  # Remove rows with missing IPs
...
def create_png_diagram(df, resource_mapping, pod_mapping):
    ...
    connected_nodes = set(df["pkt-srcaddr"].dropna()).union(set(df["pkt-dstaddr"].dropna()))
    ...
    with Diagram("EKS Architecture", show=False, outformat="png", filename="eks_architecture"):
        ...
        for _, row in df.iterrows():
            src_ip, dst_ip = row["pkt-srcaddr"], row["pkt-dstaddr"]
